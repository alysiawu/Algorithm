...
private
  ...
  def concat_can_form?(word, first_call=false)
    ...
    @trie.delete(word) if first_call

    word.length.downto(1) do |num|
      if !@trie.get(word[0...num]).empty? #if substring is in trie
        if (num == word.length) || #and if entire word param is in trie or
          ...
          @trie.add(word, word.length) if first_call
          return true
        end
      end
    end
...
@test = WordList.new
@test.load_from_file("test_words.txt")
put (@test.longest_concat_word)
